...
double* const _array_exc_feedback_synapses_delay = _dynamic_array_exc_feedback_synapses_delay.empty()? 0 : &_dynamic_array_exc_feedback_synapses_delay[0];
const size_t _numdelay = _dynamic_array_exc_feedback_synapses_delay.size();
    ///// POINTERS ////////////
    ...
    double* __restrict  _ptr_array_exc_feedback_synapses_delay = _array_exc_feedback_synapses_delay;

    std::vector<double> &real_delays = _dynamic_array_exc_feedback_synapses_delay;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = exc_feedback_synapses_pre.sources.empty() ? 0 : &(exc_feedback_synapses_pre.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = exc_feedback_synapses_pre.sources.size();
    exc_feedback_synapses_pre.prepare(_n_sources,
                           _n_targets,
