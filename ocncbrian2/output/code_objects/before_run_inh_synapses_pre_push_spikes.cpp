...
double* const _array_inh_synapses_delay = _dynamic_array_inh_synapses_delay.empty()? 0 : &_dynamic_array_inh_synapses_delay[0];
const size_t _numdelay = _dynamic_array_inh_synapses_delay.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_inh_neurons_clock_dt = _array_inh_neurons_clock_dt;
    int32_t* __restrict  _ptr_array_inh_neurons__spikespace = _array_inh_neurons__spikespace;
    double* __restrict  _ptr_array_inh_synapses_delay = _array_inh_synapses_delay;

    std::vector<double> &real_delays = _dynamic_array_inh_synapses_delay;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = inh_synapses_pre.sources.empty() ? 0 : &(inh_synapses_pre.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = inh_synapses_pre.sources.size();
    inh_synapses_pre.prepare(_n_sources,
                           _n_targets,
                           real_delays_data, n_delays, sources,
                           n_synapses,
                           _ptr_array_inh_neurons_clock_dt[0]);
