...
const int64_t _source_stop = 500;
const size_t _num_source_t = 1;
const size_t _num_spikespace = 501;
const size_t _numcount = 500;
int32_t* const _array_S_i = _dynamic_array_S_i.empty()? 0 : &_dynamic_array_S_i[0];
const size_t _numi = _dynamic_array_S_i.size();
double* const _array_S_t = _dynamic_array_S_t.empty()? 0 : &_dynamic_array_S_t[0];
const size_t _numt = _dynamic_array_S_t.size();
const size_t _num_source_idx = 500;
    ///// POINTERS ////////////
        
    int32_t*   _ptr_array_S_N = _array_S_N;
    double*   _ptr_array_exc_neurons_clock_t = _array_exc_neurons_clock_t;
    int32_t* __restrict  _ptr_array_exc_neurons_i = _array_exc_neurons_i;
    int32_t* __restrict  _ptr_array_exc_neurons__spikespace = _array_exc_neurons__spikespace;
    int32_t* __restrict  _ptr_array_S_count = _array_S_count;
    int32_t* __restrict  _ptr_array_S_i = _array_S_i;
    double* __restrict  _ptr_array_S_t = _array_S_t;
    int32_t* __restrict  _ptr_array_S__source_idx = _array_S__source_idx;


    //// MAIN CODE ////////////

    int32_t _num_events = _ptr_array_exc_neurons__spikespace[_num_spikespace-1];
    ...
    if (_num_events > 0)
    {
        ...
        {
            const int _idx = _ptr_array_exc_neurons__spikespace[_j];
        ...
        if (_num_events > 0) {
            ...
            const double _source_t = _ptr_array_exc_neurons_clock_t[0];

            for(size_t _j=_start_idx; _j<_end_idx; _j++)
            {
                const size_t _idx = _ptr_array_exc_neurons__spikespace[_j];
                const size_t _vectorisation_idx = _idx;
                                
                const int32_t _source_i = _ptr_array_exc_neurons_i[_idx];
                const int32_t _to_record_i = _source_i;
                const double _to_record_t = _source_t;

                _dynamic_array_S_i.push_back(_to_record_i);
                _dynamic_array_S_t.push_back(_to_record_t);
                _ptr_array_S_count[_idx-_source_start]++;
            }
            _ptr_array_S_N[0] += _num_events;
        }
    }
...
const size_t _num_source_idx = 500;
    ...
    std::cout << "Number of spikes: " << _ptr_array_S_N[0] << endl;
}
