...
void _before_run_exc_recurrent_synapses_pre_push_spikes()
{
    ...
    ///// CONSTANTS ///////////
    const int64_t _n_sources = 500;
const int64_t _n_targets = 500;
const size_t _num_source_dt = 1;
const size_t _num_spikespace = 501;
double* const _array_exc_recurrent_synapses_delay = _dynamic_array_exc_recurrent_synapses_delay.empty()? 0 : &_dynamic_array_exc_recurrent_synapses_delay[0];
const size_t _numdelay = _dynamic_array_exc_recurrent_synapses_delay.size();
    ///// POINTERS ////////////
    ...
    double*   _ptr_array_exc_recurrent_synapses_delay = _array_exc_recurrent_synapses_delay;

    std::vector<double> &real_delays = _dynamic_array_exc_recurrent_synapses_delay;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = exc_recurrent_synapses_pre.sources.empty() ? 0 : &(exc_recurrent_synapses_pre.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = exc_recurrent_synapses_pre.sources.size();
    exc_recurrent_synapses_pre.prepare(_n_sources,
                           _n_targets,
                           real_delays_data, n_delays, sources,
                           n_synapses,
                           _ptr_array_exc_neurons_clock_dt[0]);
